...
namespace Intertazz.Formularios
{
    public partial class frmStock : Form
    {
        ...
        private void CargarConsultaInicial()
        {
            ...
            dgvStocks.DataSource = obj.ObtenerStock();
            dgvStocks.Columns["Activo"].Visible=false;
            dgvStocks.Columns["IdStock"].HeaderText = "Cod. Stock";
            dgvStocks.Columns["IdStock"].ReadOnly =true;
        }

        private void btnBuscar_Click(object sender, EventArgs e)
        {
            Stock Stock = new Stock();
            //Stock.Nombre = txtConsNombre.Text.Trim();
            Stock.IdInventario = Convert.ToInt32(txtConsCod.Text.Trim()=="" ? "0" : txtConsCod.Text.Trim());
            dgvStocks.DataSource= obj.ObtenerStock(Stock);
            dgvStocks.Columns["IdStock"].HeaderText = "Cod. Stock";
            dgvStocks.Columns["IdStock"].ReadOnly = true;
        }
        ...
        {
            Stock Stock = new Stock();
            Stock.IdInventario = Convert.ToInt32(dgvStocks.CurrentRow.Cells[0].Value.ToString());
            //Stock.Nombre = dgvStocks.CurrentRow.Cells[1].Value.ToString();
            obj.ActualizarStock(Stock);
            CargarConsultaInicial();
